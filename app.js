/* =================================================================
   CONFIGURAÇÃO  —  edite os 3 valores abaixo antes de usar
================================================================== */
const CONFIG = {
  SUPABASE_URL:  "https://chyiakakkugjmlfuzqmf.supabase.co",   // <-- troque
  ...
  COL_CNPJ_GRUPO:"cnpj_empresa",   // coluna em substabelecidos p/ o CNPJ da empresa do grupo
...
============================================================ */
const state = {
  rows: [],          // dados brutos do supabase
  empresas: [],      // empresas_grupo (para o dropdown)
  bancos: [],        // tabela bancos
  ...
  alertasVistos: false,
  painelFiltro: "TODOS",
  sortCol: null,     // coluna clicada para ordenação (nome_subs | banco | tipo_cadastro | comissao)
...
const getPendentes = () => { try{ return JSON.parse(localStorage.getItem(LS_PENDENTES)||"[]"); }catch(e){ return []; } };
const setPendentes = arr => localStorage.setItem(LS_PENDENTES, JSON.stringify(arr));
const addPendente = id => { const p=getPendentes(); if(!p.includes(id)){ p.push(id); setPendentes(p); } };

// todas as dúvidas enviadas por ESTE navegador (não some ao ser respondida)
const LS_MINHAS = "prospecta_minhas_duvidas";
const getMinhas = () => { try{ return JSON.parse(localStorage.getItem(LS_MINHAS)||"[]"); }catch(e){ return []; } };
const addMinha = id => { const p=getMinhas(); if(!p.includes(id)){ p.push(id); localStorage.setItem(LS_MINHAS, JSON.stringify(p)); } };

const LS_HISTORICO = "prospecta_notif_historico";
const getHistorico = () => { try{ return JSON.parse(localStorage.getItem(LS_HISTORICO)||"[]"); }catch(e){ return []; } };
const setHistorico = arr => localStorage.setItem(LS_HISTORICO, JSON.stringify(arr));

/* linha "de verdade": tem empresa OU cnpj (descarta padding vazio) */
...
/* Cód. sub: duas letras iniciais = UF (ex.: MG1044, SE10993, BA11606) */
function ufDoSub(r){
...
async function carregar(){
  ...
  try{
    const [resSubs, resEmp, resBancos, resAgenda] = await Promise.all([
      fetch(`${REST()}?select=*&order=id.asc`, {headers:H()}),
      fetch(`${REST_EMP()}?select=razao_social,fantasia,cnpj&order=razao_social.asc`, {headers:H()}),
      fetch(`${REST_BANCOS()}?select=*&order=nome_banco.asc`, {headers:H()}),
      fetch(`${REST_AGENDA()}?select=*&order=data.asc,hora.asc`, {headers:H()})
    ]);
    ...
    if(resEmp.ok){ state.empresas = await resEmp.json(); montarEmpresasSelect(); }
    else { console.warn("empresas_grupo:", resEmp.status, await resEmp.text()); toast("Empresas do grupo não carregaram (RLS?).","err"); }
    if(resBancos.ok){ state.bancos = await resBancos.json(); }
    else { console.warn("bancos:", resBancos.status, await resBancos.text()); }
    if(resAgenda.ok){ state.agenda = await resAgenda.json(); checarAlertas(); }
    else { console.warn("agenda:", resAgenda.status, await resAgenda.text()); }
    montarFiltros();
    aplicarFiltros();
...
/* dropdown de empresas do grupo (value = razao_social, guarda o cnpj) */
function montarEmpresasSelect(){
  const sel=$("f_razao"), atual=sel.value;
  const opts = state.empresas
    .filter(e=>norm(e.razao_social))
    .map(e=>{
...
function distintos(campo){
  return [...new Set(state.rows.filter(isReal).map(r=>norm(r[campo])).filter(Boolean))]
...
function montarFiltros(){
  ...
  const bancosCadastrados = [...new Set(
    state.bancos.map(b=>norm(b.nome_banco)).filter(Boolean)
  )].sort((a,b)=>a.localeCompare(b,'pt-BR'));
...
/* select de banco do FORM: só bancos cadastrados e ATIVOS (tabela bancos) */
function preencherBancosForm(){
  const sel=$("f_banco"); if(!sel) return;
  const atual=sel.value;
  const nomes=[...new Set(
    state.bancos
      .filter(b=>norm(b.status).toUpperCase()!=="INATIVO")
      .map(b=>norm(b.nome_banco))
      .filter(Boolean)
  )].sort((a,b)=>a.localeCompare(b,'pt-BR'));
  sel.innerHTML = '<option value="">Selecione…</option>' +
    nomes.map(v=>`<option>${escapeHtml(v)}</option>`).join("");
  if(atual && nomes.includes(atual)) sel.value=atual;
}

/* ============================================================
   FILTROS + RENDER TABELA
============================================================ */
function aplicarFiltros(){
  const q=lower($("fBusca").value), banco=$("fBanco").value,
        st=$("fStatus").value, tipo=$("fTipo").value, ger=$("fGerente").value;

  // modo consulta: só mostra a lista depois que o consultor escolhe um banco
  state.aguardandoBanco = (!state.gestor && !banco);
  if(state.aguardandoBanco){
    state.filtered=[]; state.page=1; renderTabela(); return;
  }
  ...
  state.filtered = state.rows.filter(isReal).filter(r=>{
    ...
    if(q){
      const blob=[r.nome_subs,r.cnpj_subs,r.cod_loja_banco,r.cod_substabelecido,r.cod_parceiro,r.responsavel_empresa,r.banco]
        .map(lower).join(" ");
...
/* ordenação alfabética por coluna (Empresa / Banco / Tipo / Comissão) */
...
function renderTabela(){
  const {filtered,page}=state, size=CONFIG.PAGE_SIZE;
  ...
  tb.innerHTML = slice.map(r=>{
    ...
    const nomeCell = state.gestor
      ? `<td class="empresa"><span class="linklike" data-obs="${r.id}" title="Observações">${nome}</span>${temObs(r)?' <span class="obs-dot" title="tem observação">●</span>':''}</td>`
...
function buildDonutCard(titulo, obj, opts){
  ...
  const stops = entries.map(([,n],i)=>{
    ...
    const from=acc, to=acc+(total?n/total*100:0); acc=to;
...
function renderPainel(){
  ...
  let base = reais;
  if(filtro==="ATIVO") base=ativos;
  else if(filtro==="INATIVO") base=inativos;
  else if(filtro==="PENDENTE") base=pendentes;
  else if(filtro==="EM_ANDAMENTO") base=andamento;
  else if(filtro==="INCOMPATIVEL") base=incompativeis;

  const porBanco = contarPor(base, r=>norm(r.banco), "Sem banco");
  const porUF={}, porRegiao={};
  base.forEach(r=>{
    const uf = ufDoSub(r);
    if(uf){ porUF[uf]=(porUF[uf]||0)+1; porRegiao[UF_REGIAO[uf]]=(porRegiao[UF_REGIAO[uf]]||0)+1; }
  ...
  $("viewPainel").innerHTML = `
    ...
    <div class="painel-grid">
      ${buildDonutCard("Por região (UF do Cód. sub)", porRegiao, {det:"regiao", wide:true})}
      ${buildTowerCard("Por banco", porBanco, {limite:10, det:"banco"})}
      ${buildTowerCard("Por UF", porUF, {colorful:true, limite:14, det:"uf"})}
    </div>`;
}

/* base do painel respeitando o filtro dos KPIs (mesmos números das torres) */
function painelBase(){
  const reais = state.rows.filter(isReal);
...
/* modal de detalhe: clique nas torres do Painel Sintético */
function abrirDetalhePainel(tipo, valor){
  const base = painelBase();
  state.detalheAtual = {tipo, valor};
  const badgeDe = r => badgeStatus(norm(r.status).toUpperCase());

  if(tipo==="banco"){
    const subs = base.filter(r=>(norm(r.banco)||"Sem banco")===valor)
    ...
    const subs = valor==="Não identificado"
      ? base.filter(r=>!ufDoSub(r))
      : base.filter(r=>ufDoSub(r)===valor);
    const porBanco={};
    subs.forEach(r=>{const b=norm(r.banco)||"Sem banco";porBanco[b]=(porBanco[b]||0)+1;});
    ...
    const subs = base.filter(r=>UF_REGIAO[ufDoSub(r)]===valor)
      .sort((a,b)=>norm(a.nome_subs).localeCompare(norm(b.nome_subs),"pt-BR"));
    const porUF={};
    subs.forEach(r=>{const uf=ufDoSub(r)||"—"; porUF[uf]=(porUF[uf]||0)+1;});
...
/* ============================================================
   FICHA DO SUBSTABELECIDO (modal + PDF)
============================================================ */
...
function camposFichaSub(r){
  ...
  return {
    identificacao: [
      ["Nome do sub", norm(r.nome_subs)],
      ["CNPJ do sub", norm(r.cnpj_subs)],
      ["Empresa do grupo (razão)", emp ? norm(emp.razao_social) : ""],
      ["CNPJ do grupo", norm(r[CONFIG.COL_CNPJ_GRUPO])]
    ],
    vinculo: [
      ["Banco", norm(r.banco)],
      ["Tipo de cadastro", norm(r.tipo_cadastro)],
      ["Cód. loja banco", norm(r.cod_loja_banco)],
      ["Cód. substabelecido", norm(r.cod_substabelecido)],
      ["Cód. parceiro", norm(r.cod_parceiro)],
      ["UF / Região", uf ? `${uf} · ${UF_REGIAO[uf]}` : ""]
    ],
    gestao: [
      ["Responsável (empresa)", norm(r.responsavel_empresa)],
      ["Gerente comercial", norm(r.gerente_comercial)],
      ["Comissão", norm(r.comissao)],
      ["Status", (STATUS_SUB[norm(r.status).toUpperCase()]||{}).label || norm(r.status)]
    ]
  };
}
...
function abrirFichaSub(id){
  ...
  $("subBody").innerHTML = `
    <div class="det-resumo">
      <div class="det-kpi sub-box"><b class="st-${(STATUS_SUB[stU]||{classe:"none"}).classe}">${escapeHtml((STATUS_SUB[stU]||{}).label||"—")}</b><span>status</span></div>
...
async function exportarFichaSubPDF(){
  ...
  if(!r) return;
  const btn=$("subPdfBtn"); const oldHtml=btn.innerHTML;
  ...
  try{
    ...
    const NAVY=[15,31,56], BLUE=[26,86,196], LINE=[200,212,229],
          TXT=[18,33,53], MUT=[84,104,127],
          VERDE=[14,138,95], VERM=[201,58,63], AMBAR=[160,101,20];
    ...
    doc.setFillColor(...BLUE); doc.rect(0,26,W,1.2,"F");
    doc.setTextColor(255,255,255); doc.setFont("helvetica","bold"); doc.setFontSize(15);
    doc.text("Prospecta",14,11);
    doc.setFont("helvetica","normal"); doc.setFontSize(7.5); doc.setTextColor(154,171,201);
    doc.text("G E S T Ã O   D E   S U B S T A B E L E C I D O S",14,16.5);
    doc.setFontSize(8);
    doc.text("Gerado em "+agora, W-14, 13.5, {align:"right"});
    doc.setTextColor(...BLUE); doc.setFont("helvetica","bold"); doc.setFontSize(8);
    doc.text("FICHA · SUBSTABELECIDO",14,37);
    doc.setTextColor(...TXT); doc.setFontSize(18);
    const titulo = doc.splitTextToSize(norm(r.nome_subs)||"Substabelecido", W-70);
    doc.text(titulo,14,45);
    doc.setFontSize(9); doc.setTextColor(...stCor); doc.setFont("helvetica","bold");
    doc.text(stLbl.toUpperCase(), W-14, 45, {align:"right"});
    let y = 45 + (titulo.length-1)*8 + 5;
    doc.setDrawColor(...LINE); doc.setLineWidth(.4); doc.line(14,y,W-14,y);
    y += 8;

    const secao = (nome)=>{
      if(y > H-45){ doc.addPage(); y=20; }
      doc.setTextColor(...MUT); doc.setFont("helvetica","bold"); doc.setFontSize(8);
      doc.text(nome.toUpperCase(),14,y);
      doc.setDrawColor(...NAVY); doc.setLineWidth(.6); doc.line(14,y+2,W-14,y+2);
      y += 6;
    };
    const tabelaKV = (pares)=>{
      doc.autoTable({
        ...
        theme:"grid",
        styles:{font:"helvetica",fontSize:9,textColor:TXT,lineColor:LINE,lineWidth:.2,cellPadding:2.8},
        columnStyles:{
          0:{cellWidth:58,fontStyle:"bold",textColor:MUT,fillColor:[246,249,253],fontSize:8}
        },
        margin:{left:14,right:14},
        didParseCell:d=>{
          if(d.column.index===1 && /CNPJ|Cód\./.test(String(d.row.raw[0]))) d.cell.styles.font="courier";
          if(d.column.index===1 && d.row.raw[0]==="Status"){ d.cell.styles.textColor=stCor; d.cell.styles.fontStyle="bold"; }
        }
      });
    ...
    const campos = camposFichaSub(r);
    secao("Identificação");     tabelaKV(campos.identificacao);
    secao("Vínculo bancário");  tabelaKV(campos.vinculo);
    secao("Gestão comercial");  tabelaKV(campos.gestao);
    ...
    if(obs.length){
      ...
      doc.autoTable({
        ...
        theme:"grid",
        styles:{font:"helvetica",fontSize:8.5,textColor:TXT,lineColor:LINE,lineWidth:.2,cellPadding:2.6},
        headStyles:{fillColor:NAVY,textColor:[255,255,255],fontSize:7.8,fontStyle:"bold",cellPadding:3},
        alternateRowStyles:{fillColor:[249,251,254]},
        columnStyles:{0:{cellWidth:28,fontStyle:"bold"},1:{cellWidth:30,font:"courier",fontSize:8}},
    ...
    for(let i=1;i<=total;i++){
      doc.setPage(i);
      doc.setDrawColor(...LINE); doc.setLineWidth(.3); doc.line(14,H-12,W-14,H-12);
      doc.setFont("helvetica","normal"); doc.setFontSize(7.3); doc.setTextColor(...MUT);
      doc.text("Prospecta · documento de uso interno",14,H-7.5);
      doc.text(`Página ${i} de ${total}`,W-14,H-7.5,{align:"right"});
    }
...
/* ============================================================
   EXPORTAÇÃO PDF — detalhe do painel (jsPDF + autoTable via CDN)
============================================================ */
let _pdfLibs = null;
function loadPdfLibs(){
  ...
  const load = src => new Promise((res,rej)=>{
    ...
    s.src=src; s.onload=res; s.onerror=()=>rej(new Error("Falha ao carregar biblioteca de PDF"));
    document.head.appendChild(s);
  });
  _pdfLibs = load("https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js")
...
function slugArquivo(s){
  return norm(s).normalize("NFD").replace(/[\u0300-\u036f]/g,"")
    .replace(/[^a-zA-Z0-9]+/g,"_").replace(/^_+|_+$/g,"").toLowerCase() || "detalhe";
}
...
async function exportarDetalhePDF(){
  ...
  try{
    ...
    const NAVY=[15,31,56], BLUE=[26,86,196], LINE=[200,212,229],
          TXT=[18,33,53], MUT=[84,104,127],
          VERDE=[14,138,95], VERM=[201,58,63];

    const base = painelBase();
    const filtroLbl = {TODOS:"Todos os cadastros", ATIVO:"Somente ativos",
      PENDENTE:"Somente pendentes", EM_ANDAMENTO:"Somente em andamento",
      INATIVO:"Somente inativos", INCOMPATIVEL:"Somente incompatíveis"}[state.painelFiltro||"TODOS"];
    const agora = new Date().toLocaleString("pt-BR",{timeZone:"America/Sao_Paulo",
      day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"});

    /* faixa institucional + bloco de título */
    const cabecalho = (kicker, titulo)=>{
      ...
      doc.text("Filtro do painel: "+filtroLbl, W-14, 16.5, {align:"right"});
      doc.setTextColor(...BLUE); doc.setFont("helvetica","bold"); doc.setFontSize(8);
      doc.text(kicker.toUpperCase(),14,37);
      doc.setTextColor(...TXT); doc.setFontSize(18);
      doc.text(titulo,14,45);
      doc.setDrawColor(...LINE); doc.setLineWidth(.4); doc.line(14,49,W-14,49);
    };

    /* caixas de resumo (retorna y final) */
    const resumo = (itens)=>{
      const y=55, bh=18, gap=6;
      const bw=(W-28-(itens.length-1)*gap)/itens.length;
      let x=14;
      itens.forEach(it=>{
        doc.setFillColor(246,249,253); doc.rect(x,y,bw,bh,"F");
        doc.setDrawColor(...LINE); doc.setLineWidth(.25); doc.rect(x,y,bw,bh,"S");
        doc.setFillColor(...(it.cor||BLUE)); doc.rect(x,y,bw,1.3,"F");
        doc.setTextColor(...(it.cor||TXT)); doc.setFont("helvetica","bold"); doc.setFontSize(15);
        doc.text(String(it.v), x+bw/2, y+9.5, {align:"center"});
        doc.setTextColor(...MUT); doc.setFont("helvetica","normal"); doc.setFontSize(6.8);
        doc.text(it.label.toUpperCase(), x+bw/2, y+14.6, {align:"center"});
        x+=bw+gap;
      });
      return y+bh;
    };
    ...
    let nomeArq;
    if(det.tipo==="banco"){
      const subs = base.filter(r=>(norm(r.banco)||"Sem banco")===det.valor)
        .sort((a,b)=>norm(a.nome_subs).localeCompare(norm(b.nome_subs),"pt-BR"));
      const nAtivos = subs.filter(r=>norm(r.status).toUpperCase()==="ATIVO").length;

      cabecalho("Relatório · Banco", det.valor);
      const yFim = resumo([
        {v:subs.length, label:"substabelecidos"},
        {v:nAtivos, label:"ativos", cor:VERDE},
        {v:subs.length-nAtivos, label:"demais", cor:VERM}
      ]);
      ...
      doc.autoTable(Object.assign({}, tabelaBase, {
        ...
        body: subs.map((r,i)=>[
          ...
          norm(r.gerente_comercial)||"—",
          norm(r.status).toUpperCase()||"—"
        ]),
        columnStyles:{
          0:{cellWidth:9,halign:"center",textColor:MUT},
          2:{cellWidth:34,font:"courier",fontSize:8},
          4:{cellWidth:22,font:"courier",fontSize:8},
        ...
        didParseCell: d=>{
          if(d.section==="body" && d.column.index===6){
            ...
            else d.cell.styles.textColor=MUT;
          }
        }
      }));
      ...
      const subs = det.valor==="Não identificado"
        ? base.filter(r=>!ufDoSub(r))
        : base.filter(r=>ufDoSub(r)===det.valor);
      const porBanco={};
      subs.forEach(r=>{const b=norm(r.banco)||"Sem banco";porBanco[b]=(porBanco[b]||0)+1;});
      const entries=Object.entries(porBanco).sort((a,b)=>b[1]-a[1]);
      const titulo = det.valor==="Não identificado"
        ? "UF não identificada"
        : det.valor + (UF_REGIAO[det.valor]?` · ${UF_REGIAO[det.valor]}`:"");

      cabecalho("Relatório · Unidade federativa", titulo);
      const yFim = resumo([
        {v:subs.length, label:"subs na UF"},
        {v:entries.length, label:"bancos"}
      ]);

      doc.autoTable(Object.assign({}, tabelaBase, {
        startY: yFim+7,
        head:[["#","Banco","Substabelecidos","% do total da UF"]],
        body: entries.map(([b,n],i)=>[
          i+1, b, n, subs.length? (n/subs.length*100).toFixed(1).replace(".",",")+"%":"—"
        ]),
        columnStyles:{
          0:{cellWidth:9,halign:"center",textColor:MUT},
          2:{cellWidth:36,halign:"center",fontStyle:"bold"},
          3:{cellWidth:36,halign:"center",textColor:MUT}
        }
      }));
      nomeArq = `prospecta_uf_${slugArquivo(det.valor)}.pdf`;
    }
    else{
      const subs = base.filter(r=>UF_REGIAO[ufDoSub(r)]===det.valor)
        .sort((a,b)=>norm(a.nome_subs).localeCompare(norm(b.nome_subs),"pt-BR"));
      const porUF={};
      subs.forEach(r=>{const uf=ufDoSub(r)||"—"; porUF[uf]=(porUF[uf]||0)+1;});
      const entriesUF=Object.entries(porUF).sort((a,b)=>b[1]-a[1]);
      ...
      const yFim = resumo([
        {v:subs.length, label:"subs na região"},
        {v:entriesUF.length, label:"estados"},
        {v:nAtivos, label:"ativos", cor:VERDE}
      ]);
      ...
      doc.autoTable(Object.assign({}, tabelaBase, {
        ...
        body: entriesUF.map(([uf,n],i)=>[
          i+1, uf, n, subs.length? (n/subs.length*100).toFixed(1).replace(".",",")+"%":"—"
        ]),
        columnStyles:{
          0:{cellWidth:9,halign:"center",textColor:MUT},
          2:{cellWidth:36,halign:"center",fontStyle:"bold"},
      ...
      doc.autoTable(Object.assign({}, tabelaBase, {
        ...
        body: subs.map((r,i)=>[
          i+1, norm(r.nome_subs)||"—", ufDoSub(r)||"—", norm(r.banco)||"—", norm(r.status).toUpperCase()||"—"
        ]),
        columnStyles:{
          0:{cellWidth:9,halign:"center",textColor:MUT},
...
function renderKPIs(){
  ...
  // top bancos por qtd de ativos
  const porBanco={};
  ativos.forEach(r=>{const b=norm(r.banco)||"—";porBanco[b]=(porBanco[b]||0)+1;});
  const top=Object.entries(porBanco).sort((a,b)=>b[1]-a[1]).slice(0,5);
  const maxTop=top.length?top[0][1]:1;

  // CNPJ em mais de um banco (só ativos)
  const cnpjBanco={};
  ativos.forEach(r=>{const c=norm(r.cnpj);if(!c)return;(cnpjBanco[c]=cnpjBanco[c]||new Set()).add(norm(r.banco));});
  const multiBanco=Object.values(cnpjBanco).filter(s=>s.size>1).length;

  // cadastros incompletos (sem gerente OU sem cod OU sem status)
...
/* ============================================================
   CRUD
============================================================ */
/* no editar: seleciona a razão cujo cnpj bate com o gravado; se não bater, deixa em branco (legado) */
...
/* seta o select de banco; se o sub tiver banco fora da lista cadastrada, injeta como "(atual)" pra não perder no editar */
function setBanco(val){
  const sel=$("f_banco"), v=norm(val);
  sel.querySelectorAll("option[data-legacy]").forEach(o=>o.remove());
  if(v && ![...sel.options].some(o=>o.value===v)){
    const o=document.createElement("option");
    o.value=v; o.textContent=v+" (atual)"; o.dataset.legacy="1";
    sel.appendChild(o);
  }
  sel.value=v;
}

/* seta o select de tipo; se for um valor legado fora das 5 opções, injeta pra não perder no editar */
...
function abrirForm(id){
  ...
  set("f_sub",r.nome_subs); set("f_cnpj_subs",mascaraCNPJ(r.cnpj_subs)); validarCnpjSubUI();
  set("f_cnpj",r[CONFIG.COL_CNPJ_GRUPO]); setRazaoByCnpj(r[CONFIG.COL_CNPJ_GRUPO]);
  preencherBancosForm(); setBanco(r.banco);
  setTipo(r.tipo_cadastro);
  set("f_codloja",r.cod_loja_banco); set("f_codsub",r.cod_substabelecido); set("f_codparc",r.cod_parceiro);
  set("f_resp",r.responsavel_empresa); set("f_gerente",r.gerente_comercial); set("f_comissao",r.comissao);
...
function payloadDoForm(){
  const g=el=>{const v=$(el).value.trim();return v===""?null:v;};
  return {
    nome_subs:g("f_sub"), cnpj_subs:g("f_cnpj_subs"), [CONFIG.COL_CNPJ_GRUPO]:g("f_cnpj"), banco:g("f_banco"),
    tipo_cadastro:g("f_tipo"),
    cod_loja_banco:g("f_codloja"), cod_substabelecido:g("f_codsub"), cod_parceiro:g("f_codparc"),
    responsavel_empresa:g("f_resp"), gerente_comercial:g("f_gerente"),
    comissao:g("f_comissao")   // texto, como está
  };
}

async function salvarForm(){
  const nomeSub=$("f_sub").value.trim(), cnpjSub=$("f_cnpj_subs").value.trim(),
        razao=$("f_razao").value.trim(), banco=$("f_banco").value.trim(),
        tipo=$("f_tipo").value.trim(), gerente=$("f_gerente").value.trim(),
        comissao=$("f_comissao").value.trim();
  const faltando=[];
  ...
  if(faltando.length){ toast("Obrigatórios: "+faltando.join(", "),"err"); return; }
  if(!validaCNPJ(cnpjSub)){ validarCnpjSubUI(); toast("CNPJ do sub inválido.","err"); $("f_cnpj_subs").focus(); return; }

  const body=payloadDoForm();

  // CRIAR: vai para a etapa 2 (observação). Não grava ainda.
  if(!state.editingId){
    state.obsPendingBody={...body, status:"EM_ANDAMENTO"};
    abrirObs("create", null, "");
    return;
  }

  // EDITAR: grava direto (não mexe em status nem em observacao)
  const btn=$("formSave"); btn.disabled=true; const orig=btn.textContent; btn.innerHTML='<span class="spin"></span> Salvando';
  try{
    const res=await fetch(`${REST()}?id=eq.${state.editingId}`,{method:"PATCH",headers:{...H(),Prefer:"return=representation"},body:JSON.stringify(body)});
    if(!res.ok) throw new Error(`HTTP ${res.status} — ${await res.text()}`);
    const saved=(await res.json())[0];
    const i=state.rows.findIndex(x=>x.id===state.editingId); if(i>-1) state.rows[i]=saved;
    $("formOverlay").classList.remove("show");
    montarFiltros(); aplicarFiltros(); renderKPIs();
    ...
    console.error(e); toast("Erro ao salvar. Veja o console (F12).","err");
  }finally{ btn.disabled=false; btn.textContent=orig; }
}

const STATUS_SUB = {
  ATIVO:        {label:"Ativo",         classe:"ativo"},
  PENDENTE:     {label:"Pendente",      classe:"pendente"},
  EM_ANDAMENTO: {label:"Em andamento",  classe:"andamento"},
  INATIVO:      {label:"Inativo",       classe:"inativo"}
};
...
async function mudarStatus(id,novo){
  try{
    const res=await fetch(`${REST()}?id=eq.${id}`,{method:"PATCH",headers:{...H(),Prefer:"return=representation"},body:JSON.stringify({status:novo})});
    if(!res.ok) throw new Error(`HTTP ${res.status} — ${await res.text()}`);
    const saved=(await res.json())[0];
    const i=state.rows.findIndex(x=>x.id===id); if(i>-1) state.rows[i]=saved;
    aplicarFiltros(); renderKPIs();
  ...
  }catch(e){ console.error(e); toast("Erro ao mudar status.","err"); }
}

/* popover "Alterar status" — abre junto ao botão clicado, fecha ao clicar fora/Esc */
...
function fecharMenuStatusEsc(e){ if(e.key==="Escape") fecharMenuStatus(); }
...
function abrirMenuStatus(btn, id){
  ...
  pop.id="statusPopover"; pop.className="status-popover";
  pop.innerHTML = `<div class="sp-title">Alterar status</div>` +
    Object.entries(STATUS_SUB).map(([key,cfg])=>
      `<button class="sp-opt${key===atual?" is-atual":""}" data-setstatus="${key}">
...
/* ============================================================
   DÚVIDAS AO GESTOR + NOTIFICAÇÕES
============================================================ */
async function enviarDuvida(){
  const nome=$("duvidaNome").value.trim(), msg=$("duvidaMsg").value.trim();
  if(!msg){ toast("Escreva a dúvida antes de enviar.","err"); $("duvidaMsg").focus(); return; }
  const btn=$("duvidaSend"); btn.disabled=true; const orig=btn.textContent; btn.innerHTML='<span class="spin"></span> Enviando';
  try{
    const res=await fetch(REST_MSG(),{method:"POST",headers:{...H(),Prefer:"return=representation"},
      body:JSON.stringify([{nome_enviou:nome||null, mensagem_enviada:msg}])});
    if(!res.ok) throw new Error(`HTTP ${res.status} — ${await res.text()}`);
    const saved=(await res.json())[0];
    if(saved && saved.id!=null){ addPendente(saved.id); addMinha(saved.id); }
    $("duvidaMsg").value="";
    $("duvidaOverlay").classList.remove("show");
    toast("Dúvida enviada ao gestor","ok");
  }catch(e){ console.error(e); toast("Erro ao enviar dúvida. Veja o console (F12).","err"); }
  finally{ btn.disabled=false; btn.textContent=orig; }
}

async function carregarDuvidas(){
  try{
    const res=await fetch(`${REST_MSG()}?select=*&order=id.desc`,{headers:H()});
    if(!res.ok) throw new Error(`HTTP ${res.status} — ${await res.text()}`);
    state.duvidas = await res.json();
    state.duvidasCarregadas = true;
    atualizarBadgeDuvidas();
    renderDuvidas();
  }catch(e){ console.error(e); toast("Falha ao carregar dúvidas.","err"); }
}
...
async function excluirDuvida(id){
  ...
  try{
    ...
    atualizarBadgeDuvidas(); renderDuvidas();
    toast("Dúvida apagada","ok");
  }catch(e){ console.error(e); toast("Erro ao apagar dúvida.","err"); }
}
...
async function salvarResposta(){
  ...
  if(!texto){ toast("Escreva a resposta.","err"); return; }
  const id=state.editingDuvidaId;
  const btn=$("respSalvar"); btn.disabled=true; const orig=btn.textContent; btn.innerHTML='<span class="spin"></span> Salvando';
  try{
    const res=await fetch(`${REST_MSG()}?id=eq.${id}`,{method:"PATCH",headers:{...H(),Prefer:"return=representation"},
      body:JSON.stringify({nome_respondeu:nome||null, mensagem_respondida:texto})});
    if(!res.ok) throw new Error(`HTTP ${res.status} — ${await res.text()}`);
    const saved=(await res.json())[0];
    const i=state.duvidas.findIndex(x=>x.id===id); if(i>-1) state.duvidas[i]=saved;
    atualizarBadgeDuvidas(); renderDuvidas();
    $("respOverlay").classList.remove("show");
    toast("Resposta enviada","ok");
  }catch(e){ console.error(e); toast("Erro ao salvar resposta.","err"); }
  finally{ btn.disabled=false; btn.textContent=orig; }
}
...
async function abrirMinhas(){
  ...
  try{
    const res=await fetch(`${REST_MSG()}?id=in.(${ids.join(",")})&select=*&order=id.desc`,{headers:H()});
    if(!res.ok) throw new Error(`HTTP ${res.status} — ${await res.text()}`);
    ...
    box.innerHTML=rows.map(d=>{
      ...
      return `<div class="duvida-item">
        ...
        <div class="d-msg">${escapeHtml(norm(d.mensagem_enviada))}</div>
        ${resp ? `<div class="d-resp"><b>${escapeHtml(norm(d.nome_respondeu)||"Gestor")}:</b> ${escapeHtml(resp)}</div>`
               : `<div class="d-msg" style="color:var(--muted)">Ainda sem resposta.</div>`}
      </div>`;
    }).join("");
  }catch(e){ console.error(e); box.innerHTML='<div class="d-msg" style="padding:16px 20px;color:var(--warn)">Erro ao carregar. Veja o console (F12).</div>'; }
}
...
async function verificarRespostas(){
  ...
  try{
    const res = await fetch(`${REST_MSG()}?id=in.(${pend.join(",")})&select=id,mensagem_enviada,mensagem_respondida,nome_respondeu`, {headers:H()});
    if(!res.ok) return;
    const rows = await res.json();
    ...
    if(respondidas.length){
      ...
      respondidas.forEach(r=>{
        hist.unshift({id:r.id, mensagem_enviada:r.mensagem_enviada, mensagem_respondida:r.mensagem_respondida, nome_respondeu:r.nome_respondeu, lida:false});
...
function sairGestor(){
  ...
  $("kpis").style.display="none"; $("kpis").innerHTML="";
...
============================================================ */
/* ---------- CNPJ ---------- */
function soDigitos(s){return norm(s).replace(/\D/g,"");}
...
function validaCNPJ(s){
  ...
  const dv=base=>{
    ...
    for(let i=len;i>=1;i--){sum+=base.charAt(len-i)*pos--;if(pos<2)pos=9;}
    const r=sum%11; return r<2?0:11-r;
  };
  return dv(c.slice(0,12))===+c.charAt(12) && dv(c.slice(0,13))===+c.charAt(13);
}
/* ---------- e-mail / telefone ---------- */
function validaEmail(s){return /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(norm(s));}
function validaTelefone(s){const d=soDigitos(s);return d.length===10||d.length===11;}
...
/* telefone -> link wa.me (adiciona DDI 55 se faltar) */
...
function renderBancos(){
  ...
  tb.innerHTML = list.map(b=>{
    ...
    <td><div class="rowact">
      ...
      ${st==="ATIVO"
        ? `<button class="btn sm danger" data-inativabanco="${b.id}">Inativar</button>`
        : `<button class="btn sm" data-ativabanco="${b.id}">Reativar</button>`}
    </div></td>
  </tr>`;}).join("");
}

/* Bancos no modo consulta: read-only, só busca por nome */
function renderBancosConsulta(){
  const q = lower($("fBancoConsulta").value);
  const list = state.bancos
    .filter(b=>norm(b.status).toUpperCase()!=="INATIVO")
    .filter(b=>!q || lower(b.nome_banco).includes(q));
...
async function mudarStatusBanco(id,novo){
  try{
    const res=await fetch(`${REST_BANCOS()}?id=eq.${id}`,{method:"PATCH",headers:{...H(),Prefer:"return=representation"},body:JSON.stringify({status:novo})});
    if(!res.ok) throw new Error(`HTTP ${res.status} — ${await res.text()}`);
    const saved=(await res.json())[0];
    const i=state.bancos.findIndex(x=>x.id===id); if(i>-1) state.bancos[i]=saved;
    renderBancos();
  ...
  }catch(e){ console.error(e); toast("Erro ao mudar status do banco.","err"); }
}
...
function abrirBanco(id){
  ...
  ["b_contato_h","b_email_h","b_suporte_h"].forEach(h=>{$(h).textContent="";$(h).className="hint";});
  $("bancoOverlay").classList.add("show");
}

function validarBancoUI(){
  const set=(h,ok,txt)=>{$(h).textContent=txt;$(h).className="hint "+(ok?"ok":"warn");};
  const clear=h=>{$(h).textContent="";$(h).className="hint";};
...
async function salvarBanco(){
  const nome=$("b_nome").value.trim(), gerente=$("b_gerente").value.trim(),
        contato=$("b_contato").value.trim(), email=$("b_email").value.trim(), suporte=$("b_suporte").value.trim();

  // obrigatórios + validações
  if(!nome || !gerente || !contato || !email){ toast("Preencha os campos obrigatórios (*).","err"); return; }
  if(!validaTelefone(contato)){ validarBancoUI(); toast("Número de contato inválido.","err"); $("b_contato").focus(); return; }
  if(!validaEmail(email)){ validarBancoUI(); toast("E-mail do gerente inválido.","err"); $("b_email").focus(); return; }
  if(suporte && !validaEmail(suporte)){ validarBancoUI(); toast("E-mail de suporte inválido.","err"); $("b_suporte").focus(); return; }

  const body={ nome_banco:nome, gerente_banco:gerente, contato_gerente:contato, email_gerente:email, suporte_banco:suporte||null };
  const btn=$("bancoSave"); btn.disabled=true; const orig=btn.textContent; btn.innerHTML='<span class="spin"></span> Salvando';
  try{
    let res;
    if(state.editingBancoId){
      res=await fetch(`${REST_BANCOS()}?id=eq.${state.editingBancoId}`,{method:"PATCH",headers:{...H(),Prefer:"return=representation"},body:JSON.stringify(body)});
    }else{
      res=await fetch(REST_BANCOS(),{method:"POST",headers:{...H(),Prefer:"return=representation"},body:JSON.stringify([body])});
    }
    if(!res.ok) throw new Error(`HTTP ${res.status} — ${await res.text()}`);
    const saved=(await res.json())[0];
    if(state.editingBancoId){ const i=state.bancos.findIndex(x=>x.id===state.editingBancoId); if(i>-1) state.bancos[i]=saved; }
    ...
    renderBancos();
    if(state.editingBancoId) logHist("editou_banco","bancos",saved.id,`Editou banco ${norm(saved.nome_banco)}`);
    else logHist("criou_banco","bancos",saved.id,`Criou banco ${norm(saved.nome_banco)}`);
    toast(state.editingBancoId?"Banco atualizado":"Banco criado","ok");
  }catch(e){ console.error(e); toast("Erro ao salvar banco. Veja o console (F12).","err"); }
  finally{ btn.disabled=false; btn.textContent=orig; }
}
...
/* ============================================================
   AGENDA
============================================================ */
const AG_TIPOS = { TREINAMENTO:"Treinamento", REUNIAO:"Reunião", NOVO_CADASTRO:"Novo cadastro", OUTRO:"Outro" };
function tipoLabel(t){ return AG_TIPOS[t] || t || "—"; }
...
function fmtDataCurta(dataStr){
  const s=norm(dataStr); if(!s) return "—";
  const [y,m,d]=s.split("-"); return `${d}/${m}`;
}
/* diferença em dias (data alvo - hoje), ambos tratados como data local sem hora */
function diasAte(dataStr){
  const [y,m,d]=norm(dataStr).split("-").map(Number);
  const alvo=new Date(y,(m||1)-1,d||1); alvo.setHours(0,0,0,0);
...
function buildCalendarGrid(){
  ...
  $("agMesLabel").textContent = ref.toLocaleDateString('pt-BR',{month:'long',year:'numeric'})
  ...
  const porDia={};
  state.agenda.forEach(a=>{ const k=norm(a.data); (porDia[k]=porDia[k]||[]).push(a); });
  ...
  for(let i=0;i<totalCells;i++){
    ...
    if(dayNum<1){ cellDate=new Date(year,month-1,daysInPrev+dayNum); outClass="out"; }
    else if(dayNum>daysInMonth){ cellDate=new Date(year,month+1,dayNum-daysInMonth); outClass="out"; }
...
function renderAgendaList(){
  ...
  const proximos=state.agenda
    .filter(a=>norm(a.data)>=hojeS)
    .sort((a,b)=>(norm(a.data)+norm(a.hora)).localeCompare(norm(b.data)+norm(b.hora)));
  const box=$("agendaList");
  if(!proximos.length){ box.innerHTML=`<div class="ag-empty">Nenhum compromisso futuro.</div>`; return; }
...
function payloadAgenda(){
  const g=el=>{const v=$(el).value.trim(); return v===""?null:v;};
  return {
    titulo:g("ag_titulo"), tipo:g("ag_tipo"), data:g("ag_data"), hora:g("ag_hora"),
    alerta_dias_antes:+$("ag_alerta").value || 0,
    vinculo_tipo:g("ag_vinculo_tipo"), vinculo_nome:g("ag_vinculo_nome"),
    com_quem:g("ag_com_quem"), link:g("ag_link"), observacoes:g("ag_obs")
  };
}
...
async function salvarCompromisso(){
  ...
  const btn=$("agendaSave"); btn.disabled=true; const orig=btn.textContent; btn.innerHTML='<span class="spin"></span> Salvando';
  try{
    let res;
    if(state.editingAgendaId){
      res=await fetch(`${REST_AGENDA()}?id=eq.${state.editingAgendaId}`,{method:"PATCH",headers:{...H(),Prefer:"return=representation"},body:JSON.stringify(body)});
    }else{
      res=await fetch(REST_AGENDA(),{method:"POST",headers:{...H(),Prefer:"return=representation"},body:JSON.stringify([body])});
    }
    if(!res.ok) throw new Error(`HTTP ${res.status} — ${await res.text()}`);
    const saved=(await res.json())[0];
    if(state.editingAgendaId){ const i=state.agenda.findIndex(x=>x.id===state.editingAgendaId); if(i>-1) state.agenda[i]=saved; }
    else state.agenda.push(saved);
    $("agendaOverlay").classList.remove("show");
    renderAgenda(); checarAlertas();
  ...
  }catch(e){ console.error(e); toast("Erro ao salvar compromisso. Veja o console (F12).","err"); }
  finally{ btn.disabled=false; btn.textContent=orig; }
}

async function excluirCompromisso(){
  const id=state.editingAgendaId; if(!id) return;
  if(!confirm("Excluir este compromisso?")) return;
  try{
    const res=await fetch(`${REST_AGENDA()}?id=eq.${id}`,{method:"DELETE",headers:H()});
    if(!res.ok) throw new Error(`HTTP ${res.status} — ${await res.text()}`);
  ...
  }catch(e){ console.error(e); toast("Erro ao excluir compromisso.","err"); }
}
...
function checarAlertas(){
  ...
  if(!state.gestor || !alvo.length){ box.style.display="none"; box.innerHTML=""; return; }
...
function bindAgenda(){
  ...
  $("agPrev").onclick=()=>{ state.agendaRef=new Date(state.agendaRef.getFullYear(),state.agendaRef.getMonth()-1,1); buildCalendarGrid(); };
  $("agNext").onclick=()=>{ state.agendaRef=new Date(state.agendaRef.getFullYear(),state.agendaRef.getMonth()+1,1); buildCalendarGrid(); };
  $("agHoje").onclick=()=>{ state.agendaRef=new Date(); buildCalendarGrid(); };
  $("calGrid").addEventListener("click",e=>{
    const ev=e.target.closest("[data-editagenda]");
    if(ev){ abrirCompromisso(+ev.dataset.editagenda); return; }
...
async function logHist(acao, entidade, ref_id, descricao){
  try{
    await fetch(REST_HIST(),{method:"POST",headers:{...H(),Prefer:"return=minimal"},
      body:JSON.stringify([{quem:state.gestorNome, acao, entidade, ref_id:ref_id??null, descricao}])});
  }catch(e){ console.warn("histórico:", e); } // não bloqueia a operação principal
}
...
async function renderHistorico(){
  ...
  try{
    const res=await fetch(`${REST_HIST()}?select=*&order=criado_em.desc&limit=300`,{headers:H()});
    if(!res.ok) throw new Error(`HTTP ${res.status} — ${await res.text()}`);
    state.hist=await res.json();
  }catch(e){ console.error(e); tb.innerHTML=`<tr><td colspan="4" style="color:var(--inativo)">erro ao carregar (RLS?)</td></tr>`; return; }
...
function fmtData(iso){
  if(!iso) return "—";
  const d=new Date(iso); if(isNaN(d)) return norm(iso);
  return d.toLocaleString('pt-BR',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'});
}

/* ============================================================
   OBSERVAÇÃO (blocos tipo chat, salvos como JSON na coluna observacao)
============================================================ */
function parseObs(raw){
  const s=norm(raw);
  if(!s) return [];
  try{ const v=JSON.parse(s); if(Array.isArray(v)) return v; }catch(e){}
  return [{quem:"—", texto:s, em:null}];   // legado: texto solto vira 1 bloco
}
function temObs(r){ return parseObs(r.observacao).length>0; }
function serialObs(blocks){ return blocks.length ? JSON.stringify(blocks) : null; }

function abrirObs(mode, subId){
  state.obsMode=mode; state.obsSubId=subId||null;
  ...
  $("obsArqFile").value=""; $("obsArqTitulo").value=""; $("obsArqNome").textContent="";
...
function renderObsBlocks(){
  const box=$("obsBlocks");
  if(!state.obsBlocks.length){ box.innerHTML=`<div class="obs-vazio">Nenhuma observação ainda.</div>`; return; }
...
async function persistObsEdit(){
  const id=state.obsSubId; if(!id) return;
  try{
    const res=await fetch(`${REST()}?id=eq.${id}`,{method:"PATCH",headers:{...H(),Prefer:"return=representation"},body:JSON.stringify({observacao:serialObs(state.obsBlocks)})});
    if(!res.ok) throw new Error(`HTTP ${res.status} — ${await res.text()}`);
    const saved=(await res.json())[0];
    const i=state.rows.findIndex(x=>x.id===id); if(i>-1) state.rows[i]=saved;
    aplicarFiltros();
  }catch(e){ console.error(e); toast("Erro ao salvar observação.","err"); }
}

function obsEnviar(){
  const t=$("obsInput").value.trim(); if(!t) return;
  state.obsBlocks.push({quem:state.gestorNome, texto:t, em:new Date().toISOString()});
  $("obsInput").value="";
  renderObsBlocks();
...
async function obsConcluir(){
  const btn=$("obsConcluir"); btn.disabled=true; const orig=btn.textContent; btn.innerHTML='<span class="spin"></span> Salvando';
  try{
    const body={...state.obsPendingBody, observacao:serialObs(state.obsBlocks)};
    const res=await fetch(REST(),{method:"POST",headers:{...H(),Prefer:"return=representation"},body:JSON.stringify([body])});
    if(!res.ok) throw new Error(`HTTP ${res.status} — ${await res.text()}`);
    const saved=(await res.json())[0];
    state.rows.push(saved);
    $("obsOverlay").classList.remove("show"); $("formOverlay").classList.remove("show");
    montarFiltros(); aplicarFiltros(); renderKPIs();
    logHist("criou_sub","substabelecidos",saved.id,`Criou sub ${norm(saved.nome_subs)} (${norm(saved.banco)})`);
    toast("Substabelecido criado","ok");
  }catch(e){ console.error(e); toast("Erro ao criar. Veja o console (F12).","err"); }
  finally{ btn.disabled=false; btn.textContent=orig; }
}
...
async function carregarArquivosBanco(bancoId){
  ...
  try{
    const res=await fetch(`${REST_BANCO_ARQ()}?banco_id=eq.${bancoId}&order=criado_em.desc`,{headers:H()});
    if(!res.ok) throw new Error(`HTTP ${res.status} — ${await res.text()}`);
...
function renderArquivosBanco(list){
  const el=$("passoArqList");
  if(!list.length){ el.innerHTML='<div class="dl-empty">Nenhum arquivo anexado.</div>'; return; }
...
async function anexarArquivoBanco(){
  ...
  if(!titulo){ toast("Dê um título ao anexo.","err"); $("passoArqTitulo").focus(); return; }
  if(file.size > MAX_ARQ_MB*1024*1024){ toast(`Arquivo acima de ${MAX_ARQ_MB} MB.`,"err"); return; }

  const btn=$("passoArqAdd"); btn.disabled=true; const orig=btn.innerHTML;
  ...
  try{
    ...
    const ins=await fetch(REST_BANCO_ARQ(),{method:"POST",
      headers:{...H(),Prefer:"return=representation"},
      body:JSON.stringify({banco_id:bancoId, titulo, nome_arquivo:file.name, path})});
  ...
  }catch(e){ console.error(e); toast("Erro ao anexar: "+e.message,"err"); }
...
async function excluirArquivoBanco(id, path){
  ...
  try{
    ...
    const res=await fetch(`${REST_BANCO_ARQ()}?id=eq.${id}`,{method:"DELETE",
      headers:{...H(),Prefer:"return=representation"}});
    if(!res.ok) throw new Error(`HTTP ${res.status} — ${await res.text()}`);
    const rows=await res.json();
  ...
  }catch(e){ console.error(e); toast("Erro ao excluir: "+e.message,"err"); }
}

/* ============================================================
   ANEXOS DO SUBSTABELECIDO (Supabase Storage + tabela substabelecido_arquivos)
============================================================ */
...
async function carregarArquivosSub(subId){
  ...
  try{
    const res=await fetch(`${REST_SUB_ARQ()}?substabelecido_id=eq.${subId}&order=criado_em.desc`,{headers:H()});
    if(!res.ok) throw new Error(`HTTP ${res.status} — ${await res.text()}`);
...
function renderArquivosSub(list){
  const el=$("obsArqList");
  if(!list.length){ el.innerHTML='<div class="dl-empty">Nenhum documento anexado.</div>'; return; }
  el.innerHTML=list.map(a=>`
    <div class="pa-item">
      <span class="pa-ic">${iconeArquivo(norm(a.nome_arquivo))}</span>
      <div class="pa-info">
...
async function anexarArquivoSub(){
  ...
  if(!subId){ toast("Salve o cadastro antes de anexar documentos.","err"); return; }
  if(!file){ toast("Escolha um arquivo primeiro.","err"); return; }
  if(!titulo){ toast("Dê um título ao documento.","err"); $("obsArqTitulo").focus(); return; }
  if(file.size > MAX_ARQ_MB*1024*1024){ toast(`Arquivo acima de ${MAX_ARQ_MB} MB.`,"err"); return; }

  const btn=$("obsArqAdd"); btn.disabled=true; const orig=btn.innerHTML;
  btn.innerHTML='<span class="spin"></span> Enviando…';
  try{
    const nomeSan=file.name.normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-zA-Z0-9._-]+/g,"_");
    ...
    const ins=await fetch(REST_SUB_ARQ(),{method:"POST",
      headers:{...H(),Prefer:"return=representation"},
      body:JSON.stringify({substabelecido_id:subId, titulo, nome_arquivo:file.name, path})});
    if(!ins.ok) throw new Error(`HTTP ${ins.status} — ${await ins.text()}`);
...
async function excluirArquivoSub(id, path){
  ...
  try{
    ...
    const res=await fetch(`${REST_SUB_ARQ()}?id=eq.${id}`,{method:"DELETE",
      headers:{...H(),Prefer:"return=representation"}});
    if(!res.ok) throw new Error(`HTTP ${res.status} — ${await res.text()}`);
    const rows=await res.json();
...
async function salvarPasso(){
  const id=state.editingPassoId; if(!id) return;
  const texto=$("passoTexto").value;
  const btn=$("passoSave"); btn.disabled=true; const orig=btn.textContent; btn.innerHTML='<span class="spin"></span> Salvando';
  try{
    const res=await fetch(`${REST_BANCOS()}?id=eq.${id}`,{method:"PATCH",headers:{...H(),Prefer:"return=representation"},body:JSON.stringify({passo_a_passo:texto||null})});
    if(!res.ok) throw new Error(`HTTP ${res.status} — ${await res.text()}`);
    const saved=(await res.json())[0];
    const i=state.bancos.findIndex(x=>x.id===id); if(i>-1) state.bancos[i]=saved;
  ...
  }catch(e){ console.error(e); toast("Erro ao salvar passo a passo.","err"); }
  finally{ btn.disabled=false; btn.textContent=orig; }
}

/* ============================================================
   PRODUÇÃO MENSAL (por substabelecido)
============================================================ */
...
function initProducao(){
  state.prod = state.prod || {subId:null, sub:null, dados:[], convenios:[], filtroBanco:"", filtroMes:""};
  ...
  $("prodFiltroBanco").onchange = ()=>{ state.prod.filtroBanco=$("prodFiltroBanco").value; renderSinteticoProducao(); renderTabelaProducao(); renderConvenioProducao(); };
  $("prodFiltroMes").onchange = ()=>{ state.prod.filtroMes=$("prodFiltroMes").value; renderSinteticoProducao(); renderTabelaProducao(); };
  if(!state.prod.subId){
    $("prodVazio").style.display="block"; $("prodConteudo").style.display="none";
  }
}
...
async function selecionarSubProducao(subId){
  const r = state.rows.find(x=>x.id===subId); if(!r) return;
  state.prod.subId = subId; state.prod.sub = r;
  state.prod.filtroBanco=""; state.prod.filtroMes="";
  $("prodBuscaSub").value = norm(r.nome_subs);
  $("prodSugestoes").style.display="none"; $("prodSugestoes").innerHTML="";
  ...
  $("prodTbody").innerHTML = "";
  $("prodConvKpis").innerHTML = ""; $("prodConvTbody").innerHTML = ""; $("prodConvAtualizado").textContent = "";
  ...
  try{
    const res = await fetch(`${REST_PRODUCAO()}?cod_substabelecido=eq.${encodeURIComponent(cod)}&order=mes_referencia.desc`,{headers:H()});
    if(!res.ok) throw new Error(`HTTP ${res.status} — ${await res.text()}`);
  ...
  try{
    const res2 = await fetch(`${REST_PRODUCAO_CONVENIO()}?cod_substabelecido=eq.${encodeURIComponent(cod)}&order=banco.asc,valor_bruto.desc`,{headers:H()});
...
function montarFiltrosProducao(){
  ...
  const bancos = [...new Set([
    ...dados.map(d=>norm(d.banco)),
    ...state.prod.convenios.map(c=>norm(c.banco))
  ...
  $("prodFiltroBanco").innerHTML = `<option value="">Todos os bancos</option>` +
    bancos.map(b=>`<option value="${escapeHtml(b)}">${escapeHtml(b)}</option>`).join("");
  $("prodFiltroMes").innerHTML = `<option value="">Todos os meses</option>` +
    meses.map(m=>`<option value="${escapeHtml(m)}">${escapeHtml(mesLabel(m))}</option>`).join("");
}

function dadosProducaoFiltrados(){
  return state.prod.dados.filter(d=>
    (!state.prod.filtroBanco || norm(d.banco)===state.prod.filtroBanco) &&
    (!state.prod.filtroMes || norm(d.mes_referencia)===state.prod.filtroMes)
  );
}
...
function renderSinteticoProducao(){
  ...
  dados.forEach(d=>{
    ...
    const b=norm(d.banco)||"Sem banco"; porBanco[b]=(porBanco[b]||0)+v;
    const c=norm(d.convenio)||"Não informado"; porConvenio[c]=(porConvenio[c]||0)+v;
    const f=norm(d.forma_contrato)||"Não informado"; porForma[f]=(porForma[f]||0)+v;
  });
...
function renderTabelaProducao(){
  const dados = dadosProducaoFiltrados()
    .slice()
    .sort((a,b)=> norm(b.mes_referencia).localeCompare(norm(a.mes_referencia)));
...
function limparProducao(){
  state.prod = {subId:null, sub:null, dados:[], convenios:[], filtroBanco:"", filtroMes:""};
  $("prodBuscaSub").value=""; $("prodSugestoes").style.display="none"; $("prodSugestoes").innerHTML="";
  $("prodLimpar").style.display="none";
  $("prodFieldBanco").style.display="none"; $("prodFieldMes").style.display="none";
  $("prodVazio").style.display="block"; $("prodConteudo").style.display="none";
}
...
function switchTab(t){
  ...
  const vid={welcome:"viewWelcome",subs:"viewSubs",bancosc:"viewBancosConsulta",bancos:"viewBancos",producao:"viewProducao",agenda:"viewAgenda",hist:"viewHist",duvidas:"viewDuvidas",painel:"viewPainel"}[t];
  if(vid){ const el=$(vid); if(el){ el.classList.remove("view-enter"); void el.offsetWidth; el.classList.add("view-enter"); } }
}

/* feedback visual do cnpj do sub */
function validarCnpjSubUI(){
  const v=$("f_cnpj_subs").value.trim(), h=$("cnpjSubHint");
  if(!v){ h.textContent=""; h.className="hint"; return; }
  if(validaCNPJ(v)){ h.textContent="CNPJ válido"; h.className="hint ok"; }
  else { h.textContent="CNPJ inválido"; h.className="hint warn"; }
}
...
function bindForm(){
  ...
  $("b_contato").addEventListener("input",e=>{ e.target.value=mascaraTel(e.target.value); validarBancoUI(); });
  ...
  $("bancosTbody").addEventListener("click",e=>{
    ...
    if(ed) abrirBanco(+ed.dataset.editbanco);
    else if(ps) abrirPasso(+ps.dataset.passo);
    else if(ina){ if(confirm("Inativar este banco?")) mudarStatusBanco(+ina.dataset.inativabanco,"INATIVO"); }
  ...
  $("obsInput").addEventListener("keydown",e=>{ if(e.key==="Enter" && !e.shiftKey){ e.preventDefault(); obsEnviar(); } });
  ...
  $("obsBlocks").addEventListener("click",e=>{
    const d=e.target.closest(".obs-del"); if(d) obsDel(+d.dataset.i);
  });
  // abas
...
function bind(){
  ...
  $("limparBtn").onclick=()=>{$("fBusca").value="";$("fBanco").value="";$("fStatus").value="ATIVO";$("fTipo").value="";$("fGerente").value="";aplicarFiltros();};
  $("reloadBtn").onclick=carregar;
  $("histReload").onclick=renderHistorico;
  $("prevBtn").onclick=()=>{state.page--;renderTabela();};
  $("nextBtn").onclick=()=>{state.page++;renderTabela();};

  // login
  $("gestorBtn").onclick=()=>{$("loginUser").value="";$("loginPass").value="";$("loginHint").textContent="";$("loginOverlay").classList.add("show");$("loginUser").focus();};
  $("sairBtn").onclick=()=>{ if(confirm("Deseja realmente sair da área do gestor?")) sairGestor(); };
  ...
  // Painel Sintético: KPIs clicáveis filtram os gráficos; barras abrem detalhe
  $("viewPainel").addEventListener("click",e=>{
    const d=e.target.closest("[data-det-tipo]");
    if(d){ abrirDetalhePainel(d.dataset.detTipo, d.dataset.detValor); return; }
    const c=e.target.closest("[data-filtro]");
    if(c){ state.painelFiltro=c.dataset.filtro; renderPainel(); }
  });
  ...
  $("notifClear").onclick=e=>{ e.stopPropagation(); limparHistoricoNotif(); };
  document.addEventListener("click",e=>{
    if(!e.target.closest("#notifWrap")) $("notifPanel").classList.remove("open");
  });

  // dúvida ao gestor (modal)
  $("duvidaSend").onclick=enviarDuvida;
  $("minhasBtn").onclick=abrirMinhas;
  $("duvidaBtn").onclick=()=>{ $("duvidaOverlay").classList.add("show"); $("duvidaNome").focus(); };

  // abas do consultor (sidebar)
  $("tabsCons").addEventListener("click",e=>{
    const b=e.target.closest("[data-consview]"); if(b) switchTab(b.dataset.consview);
  });
  $("wlSubs").onclick=()=>switchTab("subs");
  $("wlBancos").onclick=()=>switchTab("bancosc");
  $("wlDuvida").onclick=()=>{ $("duvidaOverlay").classList.add("show"); $("duvidaNome").focus(); };
  $("fBancoConsulta").addEventListener("input",renderBancosConsulta);
  $("duvidaMsg").addEventListener("keydown",e=>{ if(e.key==="Enter" && !e.shiftKey){ e.preventDefault(); enviarDuvida(); } });
  ...
  document.addEventListener("click",e=>{
    if(!e.target.closest(".prod-field-sub")){ const box=$("prodSugestoes"); if(box) box.style.display="none"; }
  });
  document.querySelectorAll(".overlay").forEach(o=>{
    o.addEventListener("mousedown",e=>{ o.dataset.downOnOverlay = (e.target===o) ? "1" : "0"; });
...
/* ============================================================
   FAIXA SUPERIOR: relógio + ticker de clima
============================================================ */
function initClock(){
  const fmt = new Intl.DateTimeFormat("pt-BR",{timeZone:"America/Sao_Paulo",hour:"2-digit",minute:"2-digit",second:"2-digit"});
  const tick = ()=>{ const el=$("tbClock"); if(el) el.textContent = fmt.format(new Date()); };
  tick(); setInterval(tick, 1000);
}

const MACEIO = { nome:"Maceió", lat:-9.6658, lon:-35.7353, tz:"America/Maceio" };
...
async function initTicker(){
  ...
  const url = `https://api.open-meteo.com/v1/forecast`
    + `?latitude=${MACEIO.lat}&longitude=${MACEIO.lon}`
    + `&current=temperature_2m,weather_code`
    + `&daily=weather_code,temperature_2m_max,temperature_2m_min`
    + `&forecast_days=7&timezone=${encodeURIComponent(MACEIO.tz)}`;
  try{
    ...
    if(!Number.isNaN(tAgora))
      itens.push(`<span class="tk-item"><span class="tk-city">${MACEIO.nome} agora</span><span class="tk-temp">${tAgora}°C</span> ${wmoDesc(d.current.weather_code)}</span>`);

    // próximos dias
    const dia = d?.daily;
    if(dia?.time?.length){
      const fmt = new Intl.DateTimeFormat("pt-BR",{ weekday:"long", day:"2-digit", month:"2-digit", timeZone:MACEIO.tz });
      dia.time.forEach((iso,i)=>{
        if(i===0) return; // hoje já está em "agora"
...
(function init(){
  const cfgOk = CONFIG.SUPABASE_URL && !CONFIG.SUPABASE_URL.includes("SEU-PROJETO")
             && CONFIG.SUPABASE_KEY && !CONFIG.SUPABASE_KEY.includes("SUA_ANON_KEY");
  if(!cfgOk){ $("setupScreen").style.display="block"; return; }
  $("app").style.display="flex";
  bind();
  // layout inicial do modo consulta
  $("tabs").style.display="none";
  $("tabsCons").style.display="flex";
  $("sidebar").style.display="none";
  $("filtrosTop").appendChild($("filtrosWrap"));
  ...
  initClock();
  initTicker();
  setInterval(initTicker, 15*60*1000);
  carregar();
  renderNotifPanel();
  verificarRespostas();
